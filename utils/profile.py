...
def profile(model: nn.Module, inputs, custom_ops=None, verbose=True):
    ...
    def dfs_count(module: nn.Module, prefix="\t") -> (int, int):
        ...
        for m in module.children():
            ...
            if m in handler_collection and not isinstance(m, (nn.Sequential, nn.ModuleList)): #, nn.BatchNorm2d, nn.LeakyReLU, nn.MaxPool2d, nn.Upsample
                ...
                #     total_nonzeros += non_zeros
                    nonsparcity = non_zeros / (zeros + non_zeros)
                    m_ops = m_ops*nonsparcity
                    m_params = m_params*nonsparcity
            else:
                m_ops, m_params = dfs_count(m, prefix=prefix + "\t")
            total_ops += m_ops
...
def profile3311(model: nn.Module, inputs, custom_ops=None, verbose=True):
    handler_collection = {}
    types_collection = set()
    if custom_ops is None:
        custom_ops = {}
    ...
    def dfs_count(module: nn.Module, prefix="\t") -> (int, int):
        total_ops, total_params = 0, 0
        total_ops33, total_ops11, total_params33, total_params11 = 0, 0, 0, 0
        for m in module.children():
            # if not hasattr(m, "total_ops") and not hasattr(m, "total_params"):  # and len(list(m.children())) > 0:
            #     m_ops, m_params = dfs_count(m, prefix=prefix + "\t")
            # else:
            #     m_ops, m_params = m.total_ops, m.total_params
            if m in handler_collection and not isinstance(m, (
            nn.Sequential, nn.ModuleList)):  # , nn.BatchNorm2d, nn.LeakyReLU, nn.MaxPool2d, nn.Upsample
                m_ops, m_params = m.total_ops.item(), m.total_params.item()
                ops33, params33, ops11, params11 = 0, 0, 0, 0
                if (m.total_params == 0):
                    continue
                weight = m.weight.data
                if (len(weight.size()) == 4):  # and "shortcut" not in name):  # only consider conv layers
                ...
                m_ops, m_params, ops33, params33, ops11, params11 = dfs_count(m, prefix=prefix + "\t")
            total_ops += m_ops
            total_params += m_params
            total_ops33 += ops33
            total_ops11 += ops11
            total_params33 += params33
            total_params11 += params11

            # print(prefix, module._get_name(), (total_ops, total_params))
        return total_ops, total_params, total_ops33, total_params33, total_ops11, total_params11

    total_ops, total_params, total_ops33, total_params33, total_ops11, total_params11 = dfs_count(model)
